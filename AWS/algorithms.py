...
def get_server_heuristic(server: SimulatedServer) -> float:
    """
    Calculate server attractiveness for ACO algorithm.
    
    ACO Philosophy: Favor servers with lower utilization to balance load across all servers,
    which naturally leads to higher overall system utilization compared to round-robin.
    This is the core principle of ACO - using heuristic information for better decisions.
    """
    ...
    # ACO heuristic components designed to maximize load balancing efficiency
    MIN_OFFSET = 0.01
    
    # 1. Utilization heuristic: Strongly favor underutilized servers
    # This is the PRIMARY goal of load balancing - distribute work evenly
    # Higher sensitivity = stronger preference for balanced distribution
    SENSITIVITY_UTILIZATION = 2.5  # Increased from 2.0 for better load distribution
    utilization_score = 1.0 / (utilization + MIN_OFFSET) ** SENSITIVITY_UTILIZATION
    
    # 2. Response time heuristic: Favor faster servers
    SENSITIVITY_RESPONSE = 1.5
    response_score = 1.0 / (response_time + 0.1) ** SENSITIVITY_RESPONSE
    
    # 3. Current load heuristic: Prefer servers with available capacity
    load_score = 1.0 / (current_load + MIN_OFFSET)
    
    # Weight distribution emphasizes load balancing (ACO's strength)
    # Higher weight on utilization = better load distribution = higher overall utilization
    WEIGHT_UTILIZATION = 0.5  # Increased from 0.4 - ACO excels at load balancing
    WEIGHT_RESPONSE = 0.25     # Reduced from 0.3
    WEIGHT_LOAD = 0.25         # Reduced from 0.3
...
def update_pheromones(servers: List[SimulatedServer], chosen_server_id: str, log_callback: Callable):
    ...
    ACO Learning: Reinforce paths (servers) that led to good performance.
    This positive feedback loop helps ACO learn optimal load distribution over time,
    which is why ACO typically outperforms static algorithms like Round Robin.
    """
    global global_pheromones
    
    # 1. Evaporate pheromones for all servers (forget old information gradually)
    ...
    # Better performance = more pheromone = higher chance of selection in future
