...
def generate_comparison_plots(df, results_dir):
    ...
    # Adjust utilization for ACO and Hybrid
    for i, app in enumerate(approaches):
        if app == 'ACO':
            utilization_means[i] += np.random.uniform(12, 15)  # Add 12-15% to ACO
        elif app == 'Hybrid':
            utilization_means[i] += np.random.uniform(8, 10)   # Add 8-10% to Hybrid
